...
export async function git(repo:string,args:string[],allowFailure=false){ const r=await run('git',['-C',repo,...args]); if(!allowFailure&&r.exitCode!==0) { const details=[r.stdout,r.stderr].filter(Boolean).join('\n'); throw new Error(details||`git ${args.join(' ')} failed`); } return r; }
...
export async function isClean(repo:string){ return (await git(repo,['status','--porcelain','--','.',' :(exclude).headbang'.trim()])).stdout.trim()===''; }
