...
test('real stdio MCP supports protocol-only discovery, resources, and confirmed mutation',async()=>{
  ...
  try{
    ...
    const tools=await request('tools/list'),names=tools.result.tools.map(x=>x.name);assert.ok(names.includes('headbang_flow_init'));assert.ok(names.includes('headbang_package_publish'));assert.ok(names.includes('headbang_release_plan'));assert.ok(names.includes('headbang_repair'));
